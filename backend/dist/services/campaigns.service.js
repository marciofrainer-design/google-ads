 ...
 */
async function getCampaignById(campaignId, customerId) {
    const customer = (0, googleAds_1.getCustomerClient)(customerId);
    const results = await customer.query(`
    SELECT
      campaign.id,
 ...
 */
async function createCampaign(dto, customerId) {
    const customer = (0, googleAds_1.getCustomerClient)(customerId);
    ...
    const budgetResponse = await customer.campaignBudgets.create([
        {
            ...
            delivery_method: google_ads_api_1.enums.BudgetDeliveryMethod.STANDARD,
    ...
    const campaignPayload = {
        ...
        status: dto.create.status === 'PAUSED' ? google_ads_api_1.enums.CampaignStatus.PAUSED : google_ads_api_1.enums.CampaignStatus.ENABLED,
    ...
    if (dto.create.startDateTime)
        campaignPayload.start_date_time = dto.create.startDateTime;
    if (dto.create.endDateTime)
        campaignPayload.end_date_time = dto.create.endDateTime;
    const campaignResponse = await customer.campaigns.create([campaignPayload]);
 ...
 */
async function updateCampaignStatus(campaignId, status, customerId) {
    const customer = (0, googleAds_1.getCustomerClient)(customerId);
    await customer.campaigns.update([
        {
            resource_name: `customers/${customerId || process.env.GOOGLE_ADS_CUSTOMER_ID}/campaigns/${campaignId}`,
            status: status === 'PAUSED' ? google_ads_api_1.enums.CampaignStatus.PAUSED : google_ads_api_1.enums.CampaignStatus.ENABLED,
        },
    ]);
}
